...
int main() {
    ...
    RainyMemory::sort<long long>(a, 0, n - 1, [](const long long &o1, const long long &o2) -> bool { return o1 < o2; });
    RainyMemory::sort<long long>(b, 0, n - 1, [](const long long &o1, const long long &o2) -> bool { return o1 < o2; });
    for (int i = 0; i < n; i++)x += a[i] * b[i];
    for (int i = 0; i < n; i++)y += a[i] * b[n - i - 1];
    cout << x << " " << y << endl;
    return 0;
}