...
int main() {
    ...
    cin >> n;
    for (int i = 1; i <= n; i++)cin >> a[i].height, a[i].location = i;
    for (int i = 1; i <= n; i++)cin >> b[i].height, b[i].location = i;
    RainyMemory::sort(a + 1, a + n + 1);
    RainyMemory::sort(b + 1, b + n + 1);
    for (int i = 1; i <= n; i++)c[a[i].location] = b[i].location;
    ll res = 0;
    for (ll i = 1; i <= n; i++) {
        update(c[i]);
        res = (res + i - querySum(c[i])) % mod;
    }
    cout << res << endl;
    return 0;
}